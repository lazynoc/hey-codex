...
- **"Hey Jarvis"** sends Codex's own **Voice chat** hotkey (the `realtimeVoice` binding in `~/.codex/keybindings.json`). Codex gets a three-second exclusive microphone startup window, then Hey Codex listens only for the Jarvis phrase so saying it again ends the chat hands-free. The dictation phrase stays disabled while Voice Chat owns the microphone.

If Voice Chat is closed directly in Codex, Hey Codex notices without bringing Codex forward. It records the number of top-level Codex Accessibility dialogues before opening Voice Chat, then watches for the additional dialogue to disappear. The monitor reads no conversation content and treats inaccessible state as unknown instead of guessing.
...
| Accessibility | Trigger shortcuts, restore the exact focused element, and notice when Codex Voice Chat closes. |
...
7. Closing Voice Chat directly in Codex left Hey Codex showing an active state. We now reconcile that state from Codex's top-level Accessibility dialogues.
...
- `CodexVoiceChatStateProbe.swift`: read-only top-level Accessibility window count used to detect a manually closed Voice Chat.
