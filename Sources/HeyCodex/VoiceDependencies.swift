...
struct VoiceTimings: Sendable {
    ...
    /// How long Codex's realtime voice session gets exclusive microphone
    /// access before wake listening resumes. Restarting our audio engine
    /// while the session is still negotiating its input device aborts the
    /// chat, so this must comfortably outlast that startup window.
    var voiceChatResume: Duration = .seconds(3)
    var voiceChatStatePoll: Duration = .milliseconds(250)
    var voiceChatStateGrace: Duration = .seconds(5)
    ...
    static let test = VoiceTimings(
        ...
        voiceChatResume: .milliseconds(1),
        voiceChatStatePoll: .milliseconds(1),
        voiceChatStateGrace: .milliseconds(20),
...
struct VoiceDependencies {
    ...
    static func live() -> VoiceDependencies {
        ...
        return VoiceDependencies(
            ...
            codexDialogCount: { CodexVoiceChatStateProbe.dialogCount() },
