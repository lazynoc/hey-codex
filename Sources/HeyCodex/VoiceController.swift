...
final class VoiceController {
    ...
    /// True while a wake-triggered Codex voice chat is open. The mic belongs
    /// to the chat then: listening resumes late and only for the chat phrase.
    private(set) var voiceChatIsOpen = false
    ...
    private var voiceChatStateTask: Task<Void, Never>?
    private var voiceChatDialogBaseline: Int?
    ...
    init(dependencies: VoiceDependencies = .live()) {
        ...
        dependencies.listener.onError = { [weak self] message in
            guard let self, self.isEnabled,
                  self.phase == .listening || self.phase == .voiceChatActive
            else { return }
    ...
    func setEnabled(_ enabled: Bool) {
        ...
        if enabled {
            ...
            // An open chat stays open — turning us off must not end it.
            voiceChatIsOpen = false
    ...
    func testNativeDictation() {
        guard phase != .triggering, phase != .dictating, phase != .finishing,
              phase != .voiceChatActive
        else { return }
    ...
    func finishOnboardingTest() {
        let keepsVoiceChatOpen = onboardingVoiceChatIsOpen
        onboardingTestAction = nil
        completedOnboardingTest = nil
        activeOnboardingDemo = nil
        onboardingVoiceChatIsOpen = false

        if keepsVoiceChatOpen {
            voiceChatIsOpen = true
    ...
    private func startListening() {
        ...
        do {
            ...
            pendingWakeAction = nil
            phase = voiceChatIsOpen ? .voiceChatActive : .listening
        } catch {
            phase = .error(error.localizedDescription)
        }
    }
    ...
    private var wakeBindings: [WakePhraseBinding] {
        ...
        // While the chat is open the mic is engaged: only the voice chat
        // phrase stays live (to end it) — dictation must not trigger.
        if voiceChatIsOpen {
            let chatPhrase = voiceChatPhrase.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !chatPhrase.isEmpty else { return [] }
            return [WakePhraseBinding(phrase: chatPhrase, action: .voiceChat)]
        }
    ...
    func refreshWakeBindings() {
        guard isEnabled, phase == .listening || phase == .voiceChatActive else { return }
        dependencies.listener.stop()
        startListening()
    }

    private func wakePhraseHeard(_ action: WakeAction) {
        guard phase == .listening || phase == .voiceChatActive else { return }
        // With the mic engaged by an open chat, only its own phrase may act.
        if phase == .voiceChatActive, action != .voiceChat { return }
        playCue(.wakeHeard)
    ...
    private func triggerRealtimeVoice() {
        let isClosing = voiceChatIsOpen
        let openingDialogBaseline = isClosing ? nil : dependencies.codexDialogCount()
        stopVoiceChatStateMonitor(clearBaseline: !isClosing)
        dependencies.listener.stop()
        handoffTask?.cancel()
        completionTask?.cancel()
        ...
        let handoffDelay = dependencies.timings.handoffDelay
        // Opening: give the realtime session exclusive mic time to start,
        // or restarting our engine aborts the chat before it goes live.
        let settleDelay = isClosing
            ? dependencies.timings.completionSettle
            : dependencies.timings.voiceChatResume
        handoffTask = Task { @MainActor [weak self] in
            try? await Task.sleep(for: handoffDelay)
            guard !Task.isCancelled, let self else { return }
            ...
            do {
                ...
                if isClosing {
                    self.startVoiceChatStateMonitor(
                        baseline: self.voiceChatDialogBaseline
                    )
                }
                self.phase = .error(error.localizedDescription)
                return
            ...
            self.voiceChatIsOpen = !isClosing
            if isClosing {
                self.stopVoiceChatStateMonitor()
            } else {
                self.startVoiceChatStateMonitor(baseline: openingDialogBaseline)
            }
    ...
    private func startVoiceChatStateMonitor(baseline: Int?) {
        voiceChatStateTask?.cancel()
        voiceChatStateTask = nil
        voiceChatDialogBaseline = baseline

        guard let baseline else { return }

        let pollDelay = dependencies.timings.voiceChatStatePoll
        let graceDelay = dependencies.timings.voiceChatStateGrace
        voiceChatStateTask = Task { @MainActor [weak self] in
            let clock = ContinuousClock()
            let graceDeadline = clock.now.advanced(by: graceDelay)
            ...
            while !Task.isCancelled {
                ...
                guard !Task.isCancelled, let self else { return }
                guard self.voiceChatIsOpen || self.onboardingVoiceChatIsOpen else {
                    return
                }
                guard let currentCount = self.dependencies.codexDialogCount() else {
                    continue
                }
                ...
                if observedOpen || clock.now >= graceDeadline {
                    closedSamples += 1
                }

                guard closedSamples >= 2 else { continue }
                self.voiceChatClosedExternally()
                return
            }
        }
    }

    private func stopVoiceChatStateMonitor(clearBaseline: Bool = true) {
        voiceChatStateTask?.cancel()
        voiceChatStateTask = nil
        if clearBaseline {
            voiceChatDialogBaseline = nil
        }
    }

    private func voiceChatClosedExternally() {
        let wasOnboarding = onboardingTestAction == .voiceChat
        stopVoiceChatStateMonitor()
        voiceChatIsOpen = false
        onboardingVoiceChatIsOpen = false
        activeOnboardingDemo = nil
        ...
        startListening()
        if wasOnboarding {
            completedOnboardingTest = .voiceChat
        }
    }
