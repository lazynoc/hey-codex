...
Say **"Hey Jarvis"** to toggle Codex's realtime voice chat (it follows the **Voice chat hotkey** set in Codex Settings > Voice). Wake listening resumes automatically, so saying "Hey Jarvis" again ends the chat — fully hands-free. The phrase is editable and can be turned off in **Settings > General > Voice chat**. **Voice Chat** in the menu performs the same toggle.
