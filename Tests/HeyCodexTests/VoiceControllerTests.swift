...
struct VoiceControllerTests {
    @Test func voiceChatWakePhraseTogglesChatAndResumesListening() async {
        ...
        // but with the mic engaged only the chat phrase stays bound.
        #expect(await waitUntil { harness.controller.phase == .voiceChatActive })
    ...
    @Test func dictationCannotTriggerWhileVoiceChatIsOpen() async {
        let harness = VoiceControllerHarness.make()
        ...
        // A stray dictation wake (or a click on the dictation card) must not
        // steal the microphone from the open chat.
        harness.listener.onWakePhrase?(.dictation)
        harness.controller.testNativeDictation()
        try? await Task.sleep(for: .milliseconds(20))
        #expect(harness.trigger.startCount == 0)
        #expect(harness.controller.phase == .voiceChatActive)
    }

    @Test func endVoiceChatFromMenuClosesChatAndRestoresListening() async {
        let harness = VoiceControllerHarness.make()
        ...
        harness.listener.onWakePhrase?(.voiceChat)
        #expect(await waitUntil { harness.controller.phase == .voiceChatActive })
        #expect(harness.controller.voiceChatIsOpen)

        harness.controller.startVoiceChat()
        #expect(await waitUntil { harness.voiceChatTrigger.toggleCount == 2 })
        #expect(await waitUntil { harness.controller.phase == .listening })
        #expect(!harness.controller.voiceChatIsOpen)
    }

    @Test func closingVoiceChatDirectlyInCodexRestoresListeningState() async {
        let harness = VoiceControllerHarness.make()
        harness.codexDialogCount.value = 1
        ...
        harness.codexDialogCount.value = 2
        #expect(await waitUntil { harness.controller.phase == .voiceChatActive })
    ...
    @Test func disablingWhileVoiceChatOpenLeavesChatRunning() async {
        let harness = VoiceControllerHarness.make()
        ...
        try? await Task.sleep(for: .milliseconds(20))
        // The user's conversation is theirs — no extra toggle to close it.
        #expect(harness.voiceChatTrigger.toggleCount == 1)
        #expect(!harness.controller.voiceChatIsOpen)
    }
    ...
    @Test func onboardingVoiceChatWaitsForExclusiveMicWindowBeforeResumingListener() async {
        let harness = VoiceControllerHarness.make { dependencies in
            dependencies.timings.completionSettle = .milliseconds(1)
            dependencies.timings.voiceChatResume = .milliseconds(100)
        }
        ...
        #expect(await waitUntil { harness.voiceChatTrigger.toggleCount == 1 })
        try? await Task.sleep(for: .milliseconds(20))
        #expect(harness.controller.phase == .triggering)
        #expect(!harness.listener.isListening)
